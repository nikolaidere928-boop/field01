...
Это короткая практическая инструкция: что делать руками после первой публикации проекта наружу. GitHub/GitLab repository уже создан; теперь не нужно создавать новый репозиторий, нужно аккуратно синхронизировать текущий публичный пакет и запросить первую осторожную критику.

## 0. Главное правило

Не пытайся сразу “зайти в науку”.

Первый шаг теперь — обновить уже опубликованный публичный архив и задать один осторожный вопрос.
...
## 1. Проверь уже созданный репозиторий

Открой уже опубликованный GitHub/GitLab repository и проверь, что он соответствует текущей папке `public_release/`.

Если название ещё можно уточнить, хороший вариант:
...
Visibility уже должен быть:

```text
Public
```

Но перед ссылкой на Reddit ещё раз проверь, что туда не попали личные файлы.

## 2. Синхронизируй минимальный набор

В уже опубликованном репозитории должны быть эти файлы:
...
1. Открой уже созданный GitHub repository.
2. Синхронизируй его с текущей папкой `public_release/`.
3. Проверь или создай release `v0.1-public`.
4. Открой онлайн `README.md`, `FIELD01_OVERVIEW_EN.md`, `FIELD01_REFERENCE_MAP.md` и главный PDF как незнакомый читатель.
5. Сделай первый осторожный Reddit-пост про тон и терминологию.
6. Записывай критику в `FEEDBACK_LOG.md`.
7. Исправь overview и reference map по критике.
